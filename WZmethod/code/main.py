...
def WZmethod(parser, s, variable='k', max_degree=5):
    ...
    R = '({})/({})'.format(Snum.to_string(),Sden.to_string()).replace(variable,'({}-1)'.format(variable))
...
def to_tex(F):
    ...
    while i != -1:
        ...
        to_replace_with = '\\frac{'+to_tex(F[j1:i])+'}{'+F[i+1:j2+1]+'}'
    ...
    while i != -1:
        j1 = F.find(',',i)
        j2 = F.find(']',i)
        to_replace_with = '\\binom{'+F[i+2:j1]+'}{'+to_tex(F[j1+1:j2])+'}'
    ...
    while i != -1:
        j = F.find(']',i)
        to_replace_with = '({})!'.format(F[i+2:j])
    ...
    while i != -1:
        ...
        j2 = F.find(']',i)
        use_paran = j1-(i+2) > 1
        to_replace_with = '{}{}{}^'.format('(' if use_paran else '',F[i+2:j1],')' if use_paran else '') + '{' + F[j1+1:j2] + '}'
        F = F[:i] + to_replace_with +F[j2+1:]
        j = i + len(to_replace_with) - 1
        while i > 0 and j < len(F)-1 and F[i-1] == '(' and F[j+1] == ')':
            F = F[:i-1] + F[i:j+1] + F[j+2:]
...
def remove_unnecessary_paran(s):
    pass
...
def write_tex(parser,s,variable,outfile='../texs/proofs/proof.tex'):
    f = open(outfile,'w')
    f.write(tex_header)
...
if __name__ == '__main__':
    ...
    for i,ss in enumerate(s):
        ...
        write_tex(parsers.latex2equation_parser,ss,'k','../texs/proofs/proof{}.tex'.format(num))
        print('==============================================')
