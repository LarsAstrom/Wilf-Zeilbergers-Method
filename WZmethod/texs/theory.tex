...
\subsection{Polynomial structure and definitions}\label{Sub: polystruct}
...
\begin{algorithm}[H]
  \caption{Addition}
  \inp{Polynomials $f$ and $g$, with variable setup $\vec{x}$}
...
\begin{algorithm}[H]
  \caption{Negatation}
  \inp{Polynomial $f$ with variable setup $\vec{x}$}
...
\begin{algorithm}[H]
  ...
  \inp{Polynomials $f$, with variable setup $\vec{x}$, variable $v\in\vec{x}$ and value $y$}
...
\begin{algorithm}[H]
  ...
  \label{Alg: Roots}
  \inp{Polynomial $f$, with variable setup $\vec{x}=(x_1,\ldots,x_m)$}
...
\subsection{Complicated operations -- division and GCD}\label{Ch: div,mod,gcd}\label{Sub: diffop}
...
\begin{algorithm}[H]
  \caption{Division}
  \inp{Polynomial $a$ and $b$, with variable setup $\vec{x}=(x_1,\ldots,x_m)$}
  \outp{Polynomials $q,r$ with variable setup $\vec{x}=(x_1,\ldots,x_m)$ and polynomial $f$ with variable setup $\vec{x}=(x_2,\ldots,x_m)$ such that $f\cdot a=q\cdot b + r$ and $deg(r)<deg(b)$}
  \begin{algorithmic}[1]
    \Procedure{Divide}{$a,b$}
      ...
      \State $r_0\gets$ \Call{Subtract}{\Call{Multiply}{$f_0,a$},\Call{Multiply}{$q_0,b$}}
      \State $q_1,r_1,f_1 \gets$ \Call{Divide}{$r_0,b$}
      \State $q\gets $ \Call{Add}{\Call{Multiply}{$f_1,q_0$} ,$q_1$}
      \State $r\gets r_1$
      \State $f\gets$ \Call{Multiply}{$f_0,f_1$}
      \State \Return $(q,r,f)$
    \EndProcedure
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  ...
  \inp{Polynomial $a$ and $b$, with variable setup $\vec{x}=(x_1,\ldots,x_m)$}
  \outp{Polynomial $g$ with variable setup $\vec{x}=(x_1,\ldots,x_m)$ such that
...
\begin{algorithm}[H]
  ...
  \inp{Polynomial $a$ with variable setup $\vec{x}=(x_1,\ldots,x_m)$}
  \outp{Polynomial $g$ with variable setup $\vec{x}=(x_2,\ldots,x_m)$ such that $g$ is the greatest common divisor of all coefficients $a_i$ in $a$}
  \begin{algorithmic}[1]
...
Wilf-Zeilberger's method is a method to show an equality by using a certifying pair, or a proof certificate. The method can just be used to show an identity that is given, and cannot itself come up with a guess for what a sum would be (see \reference{wilf}). This means that the method can be used to solve problems like
\begin{example}
...
We will now give a general outline of how a proof using Wilf-Zeilberger's method works. The steps in the proof\reference{wilf} are that assume we want to prove
...
We can formalize the concepts in the method like this (see \reference{wilf}).
\begin{definition}
...
Gosper's algorithm is an algorithm that can be used to find a sum when certain conditions are fulfilled for the sum, and was presented by Gosper in 1978 (see \reference{gosper}). The setting of the problem that is solved using the algorithm is that we want to find $S_k$ where
...
In order to make the thesis more easily readable and for completeness, we will provide a quite thorough derivation of the algorithm -- even though it is perfectly described in the original paper by Gosper from 1978 (see \reference{gosper}). This will mostly focus on the steps of the algorithm, but also include the proofs of claims that are made during the steps. Firstly we will describe the main steps and thereafter we will provide the proofs.
...
Now, we have quite a few details to derive in each step -- both how the step is performed more precisely and furthermore explaining why all the polynomials actually need to be polynomials and not quotients of polynomials instead.
...
\subsection{How to get polynomial $f$ in equation \eqref{Eq: Theory,Gosper,step2}}\label{Sub: getf}
The proof provided here is just due to Gosper's paper (see \reference{gosper}) and mostly very similar. First of all, we need to prove that $f$ is a polynomial and not a quotient of polynomials.
\begin{theorem}
...
Then from this we can derive what the polynomial $f_{k-1}$ is. First we write the polynomial $f_{k-1}$ on the same form as in \eqref{Eq: Theory,general polynomial} but with other coefficients:
