...
\end{equation}
This is fairly simple if the polynomial only has one variable. As soon as we introduce polynomials with more than one variable -- which is the case in all examples where \WZ is used -- it gets a bit more complicated. The way we will visualize polynomials of more than one variable is to try to keep the very same structure as in \ref{Eq: Theory,polynomials,general polynomial}, namely we store a polynomial $p(n)$ as a list of coefficients $[a_0,a_1,\ldots,a_d]$, but instead of having $a_i$ just representing integers they are instead themselves polynomials. In order to show how this can be done, lets show an example.
...
\begin{algorithm}[H]
  ...
  \end{algorithmic}
\end{algorithm}
These procedure descriptions conclude a quite theory heavy section about polynomials. Next will follow \WZ, and will decribe how the method works.

\section{\WZ}
\WZ is a method to show an equality by using a certifying pair, or a proof certificate. The method can just be used to show an identity that is given, and cannot itself come up with a guess for what a sum would be. This means that the method can be used to solve problems like
\begin{example}
...
First of all we need to show the connection between the formulation of \WZ and how Gosper's algorithm takes part in it. Gosper's algorithm will solve the third step in \WZ, namely to find a $G(n,k)$ such that conditions \ref{Eq: Theory,first condition} and \ref{Eq: Theory,second condition} will be fulfilled. The first condition is that
\begin{equation}
  F(n+1,k)-F(n,k) = G(n,k+1)-G(n,k).
\end{equation}
...
\begin{enumerate}[i)]
  ...
  \item There does not exist a solution, hence the identity cannot be shown by \WZ.
\end{enumerate}
