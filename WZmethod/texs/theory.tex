...
\begin{algorithm}[H]
  ...
  \outp{Polynomial $g$ with variable setup $\vec{x}=(x_2,\ldots,x_m)$ such that $g$ is the greatest common divisor of all coeffiecients $a_i$ in $a$}
  \begin{algorithmic}[1]
...
We can formalize the concepts in the method like this.\reference{wilf}
\begin{definition}
...
Gosper's algorithm is an algorithm that can be used to find a sum when certain conditions are fulfilled for the sum, and was presented by Gosper in 1978.\reference{gosper} The setting of the problem that is solved using the algorithm is that we want to find $S_k$ where
...
In order to make the thesis more easily readable and for completeness, we will provide a quite thorough derivation of the algorithm -- even though it is perfectly described in the original paper by Gosper from 1978.\reference{gosper} This will mostly focus on the steps of the algorithm, but also include the proofs of claims that are made during the steps. Firstly we will describe the main steps and thereafter we will provide the proofs.
